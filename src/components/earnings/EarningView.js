...
const EarningView = (props) => {
  ...
  useEffect( () => {
    ...
    if( id !== "create" ) {
      ...
      .then( async (earning) => { 
        ...
        try { const field = await fieldService.getById(earning.category);  categoryName = field.name; }
        catch(error){ console.log(error)};
        try {  const user = await accountService.getById(earning.user_id);  userName = user.name; }
        catch(error){ console.log(error)};
        try { const tutor = await tutorService.getById(earning.tutor_id); tutorName = tutor.name; }
        catch(error){ console.log(error)};

        setEarning(earning); console.log(earning); setOpen(false); 
        if(earning.image_name) setFiles(earning.image_name.split(','));

        const data = { title: earning.title, no_of_hours: earning.no_of_hours, category: categoryName, budget: earning.budget,
           status: earning.status, date_time: earning.date_time, student: userName, tutor: tutorName
        };
        setData(data);
      })    
      .catch( error => console.log(error) );
  ...
  return (
    <>
      <Card>
        <CardHeader subheader="Earning details" title="Earning" />
        <Divider />
        <CardContent>
          {data && convertToGrid(data, 6) }
          {data && convertToGrid({description: earning.description }, 12) }
