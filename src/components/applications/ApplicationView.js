...
const ApplicationView = (props) => {
  ...
  useEffect( () => {
    ...
    if( id !== "create" ) {
      ...
      .then( async (application) => { 
        const field = await fieldService.getById(application.category);
        setApplication(application); console.log(application); setOpen(false); 
        if(application.documents) setFiles(application.documents.split(','));

        const data = { name: application.name, email: application.email, application_status: application.application_status, 
          category: field.name
        };
        setData(data);
      })    
  ...
  return (
    <>
      <Card>
        <CardHeader subheader="Tutor Application" title="Application" />
        <Divider />
        <CardContent>
          {data && convertToGrid(data, 6) }
          {data && convertToGrid({description: application.description, experience: application.experience, }, 12) }
